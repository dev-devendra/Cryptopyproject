...
def app_nav():
    ...
    format_item = Menu(menu)
    format_item.add_command(label='Font')
    menu.add_cascade(label="Format", menu=format_item)

    view_item = Menu(menu)
    view_item.add_command(label='Zoom')
    view_item.add_command(label='Status bar')
    menu.add_cascade(label="View", menu=view_item)

    help_item = Menu(menu)
    help_item.add_command(label='View help')
    help_item.add_command(label='Send feedback')
...
def my_portfolio():
    api_request = requests.get("https://pro-api.coinmarketcap.com/v1/cryptocurrency/listings/latest?start=1&limit=3000&convert=USD&CMC_PRO_API_KEY=772d3134-0d09-4999-895c-eeee78065f46")
